...
{
    connection.query(`SELECT first_name AS "First Name", last_name AS "Last Name", title AS Title FROM employee LEFT JOIN role ON employee.role_id = role.id WHERE ? ORDER BY first_name`, {"employee.manager_id": managerId}, (err, res) => { console.table(res); callback() });
}

//Returns all departments.
function getAllDepartmentsQuery(callback)
{
    connection.query(`SELECT * FROM department ORDER BY name`, (err, res) => { callback(res); });
}

//Returns all managers.
function getAllManagersQuery(callback)
{
    connection.query(`SELECT DISTINCT e1.id, e1.first_name, e1.last_name FROM employee e1, employee e2 WHERE e1.id = e2.manager_id ORDER BY e1.first_name`, (err, res) => { callback(res); });
}

//Returns all roles.
function getAllRolesQuery(callback)
{
    connection.query(`SELECT id, title FROM role ORDER BY title`, (err, res) => { callback(res); });
}
...
{
    connection.query(`SELECT id, first_name, last_name FROM employee ORDER BY first_name`, (err, res) => { callback(res) });
}
